        ...
        for node_gene1 in self.minimal_node_genes:
            for node_gene2 in self.minimal_node_genes:
                ...
                if (node_gene1.type == NodeType.INPUT) and (node_gene2.type == NodeType.OUTPUT):
                    self.minimal_connection_genes.put(self.history_marking.get_connection_gene(node_gene1.innovation_number, node_gene2.innovation_number))

    def get_history_marking(self):
        return self.history_marking

    def get_random_genome(self):
        return Genome(self.minimal_node_genes.clone(),self.minimal_connection_genes.clone(),self.history_marking).mutate()
